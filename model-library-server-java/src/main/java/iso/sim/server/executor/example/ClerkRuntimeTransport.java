...
public record ClerkRuntimeTransport(
    String runId,
    String receiverId,
    String coordinatorReceiverId,
    KafkaConfigurationDto kafka
) {
    public static ClerkRuntimeTransport from(RunExecutionContext context) {
        return new ClerkRuntimeTransport(
            context.getRunId(),
            context.getRequest().getSimulation().getModelInstanceId(),
            context.getRequest().getSimulation().getCoordinatorId(),
            context.getRequest().getKafka()
        );
    }
}
